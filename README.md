...
See [`docs/phase5.md`](docs/phase5.md) for the locked plan + decisions. Phase 5 is closed; Phase 6 is now in progress — see [`docs/phase6.md`](docs/phase6.md) for the locked plan.

### Phase 6 — Conjunction-replay showcase (🚧 in progress)
...
| 2. HUD overlay + replay timeline | ⏳ next | Live miss distance + countdown, ▶ play/seek controls, both-sat ribbons, both-sat marquee, dim catalog. |
| 3. Entry points + sharing | ⏳ pending | "▶ Replay" links from `/text/conjunctions` rows + Atom events feed; existing chunk-6 Share button reuses unchanged. |
| 4. Tests + Phase 6 close | ⏳ pending | Vitest state-machine coverage, Playwright e2e of full replay flow, README closes Phase 6. |
...
```bash
# Schema management
make migrate                          # apply migrations (13 total: 6 Phase 1 + 5 Phase 2 chunk 1 + 2 Phase 4 chunks 1+3)
...
5. Add cron entries (Phase 2 chunks 1–6 ingest schedule + cache pruning):
   ...
   */15 * * * *  cd ~/sat.trackr.live && make ingest-ovation    >> storage/logs/cron.log 2>&1
   ...
   45 4 * * *    cd ~/sat.trackr.live && make sitemap-build     >> storage/logs/cron.log 2>&1
